...
@Component({
    selector: 'OutstandingIncomeRptForm',
    templateUrl: 'OutstandingIncomeRptForm.html',
    moduleId: module.id,
    providers: [ReportService],
})
export class OutstandingIncomeRptForm implements OnInit {
    ...
    RunReport() {
        this.loader.ShowLoader();
        this.reportservice.OutstandingRpt(this.model).then(m => {
            var result = JSON.parse(m._body);
            if (result.IsSuccess) {
                this.DataList = result.ResultSet.OutstandingList;
                this.loader.HideLoader();
            } else
                this.loader.HideLoader();
        });
    ...
    public DownloadPDF(): void {
        ...
        html2canvas(data).then(canvas => {
            ...
            pdf.save('OutstandingIncome.pdf');
            this.loader.HideLoader();
        });
    }
    public DownloadExcel() {
        ...
        a.download = 'OutstandingIncome.xls';
        a.click();
    }
}
