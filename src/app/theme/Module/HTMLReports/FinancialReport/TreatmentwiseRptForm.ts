...
@Component({
    selector: 'TreatmentwiseRptForm',
    templateUrl: 'TreatmentwiseRptForm.html',
    moduleId: module.id,
    providers: [ReportService],
})
export class TreatmentwiseRptForm implements OnInit {
    ...
    RunReport() {
        this.loader.ShowLoader();
        this.reportservice.TreatmentwiseRpt(this.model).then(m => {
            var result = JSON.parse(m._body);
            if (result.IsSuccess) {
                this.total = 0;
                this.DataList = result.ResultSet.TreatmentList;
                this.DataList.forEach(x => {
                    this.total += x.Totalfee
                }); 
                this.loader.HideLoader();
            } else
                this.loader.HideLoader();
