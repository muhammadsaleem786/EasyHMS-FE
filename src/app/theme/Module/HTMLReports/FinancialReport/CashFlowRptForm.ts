...
export class CashFlowRptForm implements OnInit {
    ...
    ngOnInit() {
        this.TypeList.push({ "value": 0, "Name": "Show All" }, { "value": 1, "Name": "Income only" }, { "value": 2, "Name": "Expenses only" }, { "value": 3, "Name": "Professional Fees only" })
        this.model.Type = 0;
        var date = new Date();
        ...
        this.reportservice.CashFlowRpt(this.model).then(m => {
            ...
            if (result.IsSuccess) {
                ...
                this.DataList.forEach(x => {
                    if (x.Description == "Professional Fees" || x.Description == "Income")
                        this.totalIncome += x.Amount
                    if (x.Description == "Expenses")
