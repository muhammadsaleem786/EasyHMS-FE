...
@Component({
    selector: 'DoctorswiseComponentRptForm',
    templateUrl: 'DoctorswiseComponentRptForm.html',
    moduleId: module.id,
    providers: [ReportService],
})
export class DoctorswiseComponentRptForm implements OnInit {
    ...
    RunReport() {
        ...
        if (this.model.DoctorName == null || this.model.DoctorName == "")
            this.model.DoctorId = null;
        this.reportservice.DoctorswiseFeeRpt(this.model).then(m => {
            var result = JSON.parse(m._body);
            if (result.IsSuccess) {
                this.total = 0;
                this.DataList = result.ResultSet.FeeList;
                this.DataList.forEach(x => {
                    this.total += x.Price - x.Discount
                }); 
                this.loader.HideLoader();
            } else
                this.loader.HideLoader();
