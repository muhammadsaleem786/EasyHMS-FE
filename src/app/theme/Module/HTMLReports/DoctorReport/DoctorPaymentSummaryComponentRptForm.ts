...
@Component({
    selector: 'DoctorPaymentSummaryComponentRptForm',
    templateUrl: 'DoctorPaymentSummaryComponentRptForm.html',
    moduleId: module.id,
    providers: [ReportService],
})
export class DoctorPaymentSummaryComponentRptForm implements OnInit {
    ...
    RunReport() {
        this.loader.ShowLoader();
        if (this.model.DoctorName == null || this.model.DoctorName == "")
            this.model.DoctorId = null;
        this.reportservice.DoctorswiseSummaryPaymentRpt(this.model).then(m => {
            var result = JSON.parse(m._body);
            if (result.IsSuccess) {
                this.total = 0;
                this.DataList = result.ResultSet.FeeList;
