...
@Component({
    selector: 'PatientDetailComponentForm',
    templateUrl: 'PatientDetailComponentForm.html',
    moduleId: module.id,
    providers: [ReportService],
})
export class PatientDetailComponentForm implements OnInit {
    @ViewChild('TABLE') TABLE: ElementRef;
    public model = new ReportModel();
    public Rights: any;
    public Keywords: any[] = [];
    public PayrollRegion: string;
    public DataList: any[]; public CompanyObj: any;
    public CompanyName: string; public Form1: FormGroup;
    ...
    RunReport() {
        this.loader.ShowLoader();
        this.reportservice.PatientDetailRpt(this.model).then(m => {
            var result = JSON.parse(m._body);
            if (result.IsSuccess) {
                this.DataList = result.ResultSet.patient;
