...
declare var $: any;
@Component({
    selector: 'AppointmentComponentReportForm',
    templateUrl: 'AppointmentComponentReportForm.html',
    moduleId: module.id,
    providers: [ReportService],
})
export class AppointmentComponentReportForm implements OnInit {
    ...
    RunReport() {
        ...
        if (this.model.PatientName == null || this.model.PatientName == "")
            this.model.PatientId = null;
        this.reportservice.AppointmentRpt(this.model).then(m => {
            var result = JSON.parse(m._body);
            if (result.IsSuccess) {
                this.DataList = result.ResultSet.AppointList;
