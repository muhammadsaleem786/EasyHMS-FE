...
@Component({
    selector: 'FollowupComponentRptForm',
    templateUrl: 'FollowupComponentRptForm.html',
    moduleId: module.id,
    providers: [ReportService],
})
export class FollowupComponentRptForm implements OnInit {
    ...
    RunReport() {
        this.loader.ShowLoader();
        this.reportservice.FollowupRpt(this.model).then(m => {
            var result = JSON.parse(m._body);
            if (result.IsSuccess) {
                this.DataList = result.ResultSet.followuplist;
                this.loader.HideLoader();
            } else
                this.loader.HideLoader();
        });
