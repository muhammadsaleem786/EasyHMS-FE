...
@Component({
    templateUrl: './AdmitBillSummeryComponentForm.html',
    moduleId: module.id,
    providers: [AppointmentService, BillingService, PatientService, AdmitService],
})

export class AdmitBillSummeryComponentForm implements OnInit {
    ...
    BillingGridList() {
        this.loader.ShowLoader();
        this._AppointmentService.GetIPDBillingList(this.PatientId, this.AdmitId).then(m => {
            this.BillingList = m.ResultSet.result.Table;
            this.loader.HideLoader();
        });
    }
    DischargePatient() {
        let Id = parseInt(localStorage.getItem('AdmitId'));
        this.loader.ShowLoader();
