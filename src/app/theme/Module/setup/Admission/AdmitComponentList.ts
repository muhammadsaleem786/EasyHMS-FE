...
export class AdmitComponentList {
        ...
        this._AdmitService.GetDischargRpt(AdmissionId, PatientId).then(m => {
            if (m.IsSuccess) {
                ...
                if (m.ResultSet.Detaildata.length != 0) {
                    ...
                    //localStorage.setItem("PatientId", PatientId);
                    //localStorage.setItem("AdmitId", AdmissionId);                  
    ...
    SaveOrUpdateDischarge(isValid: boolean): void {
        if (isValid) {
            this.submitted = false;
            this.loader.ShowLoader();
            this.model.AdmissionId = parseInt(localStorage.getItem("PatientID"));
            this.model.PatientId = parseInt(localStorage.getItem("AdmissionId"));
