...
export class EmployeeComponent implements OnInit, OnDestroy {
   ...
   // public VacSickLeaveModel = new VacationSickLeaveModel();
    ...
    constructor(public _fb: FormBuilder, public _router: Router, public _employeeService: EmployeeService, public commonservice: CommonService
...
//, public _vacationSickLeaveService: VacationSickLeaveService
    ) {
        ...
        if (this.IsEdit) {
            ...
            if (this.Countries.length > 0) {
                if (this.PayrollRegion == 'SA') {
                    ...
                    //this.locationModel.CountryID = 2;
                }
    ...
    */
    onStepChange(event: any) {
        ...
        else if (event.selectedIndex == 3 && !this.IsEdit) {
            //if Leave Wizard Selected Index = 3 then 
            this.IsLeaveWizardViewed = true;
            this.IsPayWizardViewed = true;
        }
        else if (event.selectedIndex == 4 && !this.IsEdit) {
            //if Docs Wizard Selected Index = 4 then 
            this.IsDocsWizardViewed = true;
            this.IsLeaveWizardViewed = true;
            this.IsPayWizardViewed = true;
        }
        else if (event.selectedIndex == 5 && !this.IsEdit) {
            //if Dependent Wizard Selected Index = 5 then 
            this.IsDependentWizardViewed = true;
            this.IsDocsWizardViewed = true;
            this.IsLeaveWizardViewed = true;
            this.IsPayWizardViewed = true;
        }
        else if (event.selectedIndex == 6 && !this.IsEdit) {
            //if Review Wizard Selected Index = 6 then 
            this.IsDependentWizardViewed = true;
            this.IsDocsWizardViewed = true;
            this.IsLeaveWizardViewed = true;
            this.IsPayWizardViewed = true;
