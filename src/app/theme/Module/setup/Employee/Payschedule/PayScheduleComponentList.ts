...
@Component({
    moduleId: module.id,
    templateUrl: 'PayScheduleComponentList.html',
    providers: [PayscheduleService],
})

export class PayScheduleComponentList {

    public Id: string;
    public PModel = new PaginationModel();
    public PConfig = new PaginationConfig();
    public PayScheduleList: any[] = [];
    public IsList: boolean = true;
    public ID: number = 10;
    public Rights: any;

    constructor(public _CommonService: CommonService, public loader: LoaderService, public _PayscheduleService: PayscheduleService
    ...
    ngOnInit() {
        ...
        this.PConfig.Action.ScreenName = "Pay Schedule";
        ...
        this.PConfig.Fields = [
            { Name: "ScheduleName", Title: "Name", OrderNo: 1, SortingAllow: true, Visible: true, isDate: false, DateFormat: "" },
            { Name: "PayTypeID", Title: "Pay Period", OrderNo: 2, SortingAllow: true, Visible: true, isDate: false, DateFormat: "" }
        ];
    ...
    Refresh() {
        ...
        this._PayscheduleService
            .GetList(this.PModel.CurrentPage, this.PModel.RecordPerPage, this.PModel.VisibleColumnInfo, this.PModel.SortName, this.PModel.SortOrder, this.PModel.SearchText).then(m => {
                ...
                this.PayScheduleList = m.DataList;
    ...
    ExportData(ExportType: number) {
        this.loader.ShowLoader();
        this._PayscheduleService.ExportData(ExportType, this.PModel.CurrentPage, this.PModel.RecordPerPage, this.PModel.VisibleColumnInfo, this.PModel.SortName, this.PModel.SortOrder, this.PModel.SearchText).then(m => {
            this.loader.HideLoader();
        });
    }
    ...
    //    if (result == true) {
    //        this.loader.ShowLoader();
    //        this._PayscheduleService.Delete(id).then(m => {
    //            
