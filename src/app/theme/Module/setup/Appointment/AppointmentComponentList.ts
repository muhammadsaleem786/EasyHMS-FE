...
@Component({
    ...
    providers: [AppointmentService, ApplicationUserService, BillingService, PatientService],
})
export class AppointmentComponentList implements OnInit {
    ...
    openDoctorModal(DocContent) {
        ...
        this.Usermodel.adm_user_company[0].RoleID = 2;
        this.modalService.open(DocContent, { size: 'md' });
    }
    OpenMedicinePrescription(MedicinePresModal) {
        ...
        this._AppointmentService.PrescriptionDropdownList().then(m => {
            if (m.IsSuccess) {
                ...
                this.modalService.open(MedicinePresModal, { size: 'lg' });
            } else
                this.loader.HideLoader();
        });
    }
    ...
    constructor(public _fb: FormBuilder, private encrypt: EncryptionService, public route: ActivatedRoute, public _PatientService: PatientService, public _BillingService: BillingService, private modalService: NgbModal, public _CommonService: CommonService, public loader: LoaderService, public _AppointmentService: AppointmentService
        , public _router: Router, public toastr: CommonToastrService, public _ApplicationUserService: ApplicationUserService) {
    ...
    LoadCalendar(Date) {
        ...
        $('#calendar').fullCalendar({
                            ...
                            if (this.isDrop) {
                                ...
                                this._AppointmentService.UpdateAppointment(this.AppointmentModel).then(m => {
                                    ...
                                    if (result.IsSuccess) {
                                        ...
                                        //this.BindEvent();
                                        //let date = this._CommonService.GetFormatDate(CalendarDate);
                                        //this.AppointmentDate = CalendarDate;
                                        //this.LoadCalendar(date);
            ...
            eventClick: (event) => {
                ...
                this.OnClickAppointment(CalendarDate, CalendarTime, event.resourceId);
                //let patientlist = this.PatientList.filter(a => a.ID == Appobj[0].PatientId);
            },
    ...
    CheckDayClik(date: any, time: any) {
        ...
        //let Calendardate = this._CommonService.GetFormatDate(date._d);
        //let CurrentDate = this._CommonService.GetFormatDate(new Date());
        //if (Calendardate < CurrentDate) {
        //    this.message = "You can not change to previous date.";
        //    this.isDayClik = false;
        //}
        //else if (Calendardate === CurrentDate && time < currentTime) {
        //    this.message = "You can not add appointment in previous time in current date.";
        //    this.isDayClik = false;
        //} else
        //    this.isDayClik = true;

    }
    OnClickAppointment(date: any, time: any, doctorId: any) {
        var doctorid = this.DoctorClanderList.filter(a => a.ID == parseInt(doctorId));
        var Appobj = this.AppointmentList.filter(a => a.DoctorId == parseInt(doctorId) && a.StartDate.split("T")[1] == time);
    ...
    SaveMedicine(Checked: any, index: any) {
        if (Checked) {
            var MedicineId = this.emr_prescription_treatment_dynamicArray[index].MedicineId;
            if (MedicineId == null || MedicineId == undefined) {
                this.loader.ShowLoader();
                this.MedicineModel.Medicine = this.emr_prescription_treatment_dynamicArray[index].MedicineName;
                this._AppointmentService.MedicineSave(this.MedicineModel).then(m => {
                    var result = JSON.parse(m._body);
                    if (result.IsSuccess) {
                        this.toastr.Success(result.Message);
                        this.emr_prescription_treatment_dynamicArray[index].MedicineId = result.ResultSet.ID;
                    }
                    else
                        this.toastr.Error('Error', result.ErrorMessage);
    ...
    checkValue(rowno: any, checkedSos: any) {
        if (checkedSos) {
            this.emr_prescription_treatment_dynamicArray[rowno].IsMorning = false;
            this.emr_prescription_treatment_dynamicArray[rowno].IsEvening = false;
            this.emr_prescription_treatment_dynamicArray[rowno].IsNoon = false;
        } else {
            this.emr_prescription_treatment_dynamicArray[rowno].IsSOS = false;
        }
    }
