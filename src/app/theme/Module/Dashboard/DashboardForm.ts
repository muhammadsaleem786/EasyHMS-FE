...
export class DashboardForm {
    ...
    Refresh(fDate: any, ToDate: any) {
        ...
        this._DashboardService
            .DataLoad(fDate, ToDate).then(m => {
                ...
                this.Female = this.DeshboardData[0].Femail;
    ...
    onChangeType(Val: any) {
        ...
        this.BirthDayData = [];
        if (Val == 0) {
            this.BirthDayData = this.BirthDayList;
        }
        if (Val == 1) {
            this.BirthDayData = this.BirthDayList.filter(a => a.IsCreateAppointment == 1);
        }
        if (Val == 2) {
            this.BirthDayData = this.BirthDayList.filter(a => a.IsCreateAppointment == 0);
        }
    }
    onDateChanged() {
        this.loader.ShowLoader();
